...
J = J + ((lambda/(2*m)) * (sum(sum(Theta1(:,2:end).*Theta1(:,2:end))) + sum(sum(Theta2(:,2:end).*Theta2(:,2:end))) + sum(sum(Theta3(:,2:end).*Theta3(:,2:end)))));

delta4 = h - Y;
delta3 = (delta4*Theta3)(:,2:end) .* sigmoidGradient(h2*Theta2');
...
Theta2_grad = D3./m;
Theta3_grad = D4./m;
Theta1_grad(:,2:end) = D2(:,2:end)./m + (lambda/m) .* Theta1(:,2:end);
Theta2_grad(:,2:end) = D3(:,2:end)./m + (lambda/m) .* Theta2(:,2:end);
Theta3_grad(:,2:end) = D4(:,2:end)./m + (lambda/m) .* Theta3(:,2:end);
